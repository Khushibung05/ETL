...
def load_to_supabase(staged_path: str, table_name: str = "titanic_data"):
    ...
    try:
        ...
        for i in range(0, total_rows, batch_size):
            ...
            try:
                # ---- operate on batch (not df) AND create records after conversion ----
                for col in batch.columns:
                    if batch[col].dtype == "float64":
                        # If all values are integers, cast to Int64
                        if batch[col].dropna().apply(lambda x: x.is_integer()).all():
                            batch[col] = batch[col].astype("Int64")

                records = batch.to_dict('records')

                # ---------- MINIMAL ADDITION ----------
                # Replace any float('nan') left in records with None (JSON-safe)
                for rec in records:
                    for k, v in list(rec.items()):
                        # detect NaN floats and replace with None
